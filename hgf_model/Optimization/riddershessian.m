function [hessf, err] = riddershessian(f, x, varargin)
...
% any later version). For further details, see the file COPYING or <http://www.gnu.org/licenses/>.
    ...
    for i = 1:n
        ...
        fxi = @(xi) eval_fxi(f,x,i,xi);

        % Calculate derivative
        [hessf(i,i), err(i,i)] = riddersdiff2(fxi,x(i),options);
    end
    ...
    for i = 2:n % rows
        for j = 1:i-1 % columns
            ...
            fxixj = @(xixj) eval_fxixj(f,x,i,j,xixj);

            % Calculate cross-derivative
            [hessf(i,j), err(i,j)] = riddersdiffcross(fxixj,[x(i),x(j)],options);
            hessf(j,i) = hessf(i,j);
            err(j,i)   = err(i,j);
        end
    end
...
function val = eval_fxi(f,x,i,xi)
    xx    = x;
    xx(i) = xi;
    val   = f(xx);
end

function val = eval_fxixj(f,x,i,j,xixj)
    xx    = x;
    xx(i) = xixj(1);
    xx(j) = xixj(2);
    val   = f(xx);
