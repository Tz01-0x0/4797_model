...
| `MLTM_group_comparison.m` | Parameter-level 2 × 2 Group × Condition ANOVA (Type III SS), $t$-tests, Cohen's $d$, partial correlations $r$(AQ, $\hat\theta_p$) controlling for Condition — **produces Table 4 of the report.** |
| `MLTM_parameter_recovery.m` | Simulation–recovery on the winning model. |
| `MLTM_laplace_kappa_theta.m` | Per-subject corr($\kappa$, $\vartheta$) from the Laplace correlation matrix $C_i$ — flat-likelihood vs. ridge diagnostic. |
| `MLTM_I1_analysis.m` | Group comparison and AQ correlations on initial-condition parameters $\hat\mu_{2,c,0}$, $\hat\sigma^2_{2,c,0}$ from `I1_Both`. |
...
| 4 | `MLTM_I1_analysis` (MATLAB, after step 3) | `hgf_model/results_I1/I1_Both_MAP_estimates.csv`, group/AQ comparison report | post-hoc on initial Level-2 mean $\hat\mu_{2,c,0}$ and variance $\hat\sigma^2_{2,c,0}$ |
...
| 6 | `MLTM_laplace_kappa_theta(MLTM_options_new())` (MATLAB) | `hgf_model/results/laplace_kappa_theta.csv`, `_hist.png`, `.txt` | per-subject corr($\kappa_c$, $\vartheta_c$) flat-vs-ridge diagnostic |
...
| 8b | `MLTM_group_comparison(MLTM_options_new())` (MATLAB, in `hgf_model/`; **also runs automatically as the final step of `MLTM_main_new`**) | parameter ANOVA console output + boxplots | **Table 4** (2 × 2 Group × Condition on $\hat\theta$) |
| 9 | `MLTM_stratified_AQ_analysis` (MATLAB) | `hgf_model/results_stratified/*.csv` | condition-stratified $r$(AQ, $\hat\theta$) on Arch A and `I1_Both` |
